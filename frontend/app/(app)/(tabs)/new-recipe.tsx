...
export default function NewRecipeScreen() {
  ...
  const takePhoto = async () => {
    ...
    if (permissionResult.granted === false) {
      Alert.alert("Permissão Necessária", "É preciso permitir acesso à câmera para tirar uma foto.");
      return;
    }
    ...
    try {
      ...
      if (apiResponse.ok) {
        ...
        if (!validIngredientsFound && Array.isArray(data) && data.length > 0 && Array.isArray(data[0]?.ingredients)) {
             console.warn("Endpoint /simulate retornou um Array, usando ingredientes do primeiro item.");
             detectedIngredients = data[0].ingredients;
             validIngredientsFound = true; // Encontrou de forma alternativa
        }

        if (validIngredientsFound) {
           console.log("Ingredientes extraídos para seleção:", detectedIngredients);
           setLoading(false);
          ...
          console.error("Resposta da API /simulate OK, mas 'ingredients' não encontrado no formato esperado:", data);
          Alert.alert('Erro de Resposta', 'Formato de ingredientes inválido vindo do servidor (/simulate).');
          setLoading(false);
        }
      } else {
        ...
        Alert.alert('Erro na Extração', `O servidor retornou um erro ao extrair ingredientes (/simulate): ${apiResponse.status}.`);
        setLoading(false);
      }
      ...
      // Tratamento de erro de rede
       ...
       Alert.alert('Erro de Rede', 'Não foi possível conectar ao servidor para extrair ingredientes.');
       setLoading(false);
    }
  };
      ...
      <View style={styles.mainContent}>
        {image ? (
          <View style={styles.imagePreviewContainer}>
            ...
            {!loading && (
                 <TouchableOpacity style={styles.analyzeButton} onPress={analyzeRecipe} disabled={loading} activeOpacity={0.8}>
                   <Text style={styles.analyzeButtonText}>Analisar Ingredientes</Text>
                 </TouchableOpacity>
            )}
            {!loading && (
                 <TouchableOpacity style={styles.retakeButton} onPress={() => setImage(null)} disabled={loading}>
